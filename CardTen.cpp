...
void CardTen::SetOwner(Player* pPlayer)              //Sets all Stations with the same number to the player
{
	pOwner = pPlayer;
}
...
void CardTen::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (pOwner = NULL)
	{
		...
		if (Answer == "y")                                       //Player Buys Station
		{
			...
			pOwner = pPlayer;
			pGrid->SetAllOwnerCard10(pPlayer);
			pOut->ClearStatusBar();
			return;
		}
		if (Answer == "n")
...
void CardTen::Save(ofstream& OutFile, ObjType obj)  //Saves the GameObj parameters to the file
{
	if (obj == card)
	{
...
void CardTen::Load(ifstream& Infile, Grid* pGrid)   //Loads the GameObj parameters to the file
{
	int cellNum;
