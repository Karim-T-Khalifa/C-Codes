...
int main()
{
	...
	pOut->PrintMessage("Please Enter a number : ");
	int z = pIn->GetInteger(pOut);
	pOut->PrintMessage("You Entered: " + to_string(z));
	...
	for (int i = 0; i < 5; i++)
	{
		...
		CellPosition D = pIn->GetCellClicked();
		int D1 = D.HCell();
		int D2 = D.VCell();
		pOut->PrintMessage("The Coordinates of the cell is: (" + to_string(D1) + "," + to_string(D2) + ")");
		pIn->GetPointClicked(x, y);	//Wait for any click
	}
	...
	pOut->PrintMessage("Please Enter a Word :");
	...
	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage("Please enter a Vcell number : ");
		int k = pIn->GetInteger(pOut);
		cellpos_1.SetVCell(k);
		int R = cellpos_1.VCell();
		pOut->PrintMessage("Now the vCell is:  " + to_string(R));
		pIn->GetPointClicked(x, y);
	}

	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage("Please enter a hcell number : ");
		int k1 = pIn->GetInteger(pOut);
		cellpos_1.SetHCell(k1);
		int R1 = cellpos_1.HCell();
		pOut->PrintMessage("Now the hCell is:  " + to_string(R1));
		pIn->GetPointClicked(x, y);
	}
	...
	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage("Please enter a vCell: ");
		int K1 = pIn->GetInteger(pOut);
		pOut->PrintMessage("Please enter a hcell: ");
		int K2 = pIn->GetInteger(pOut);
		CellPosition cellpos_2(K1, K2);
		int K3 = cellpos_2.GetCellNum();
		pOut->PrintMessage("Cell Number is:  " + to_string(K3));
		pIn->GetPointClicked(x, y);
	}
	...
	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage("Please Enter a Cell Number: ");
		int S1 = pIn->GetInteger(pOut);
		CellPosition cellpos_3(S1);
		int S2 = cellpos_3.VCell();
		int S3 = cellpos_3.HCell();
		pOut->PrintMessage("vcell and hcell are: ( "  + to_string(S2) + "," + to_string(S3) + ")");
		pIn->GetPointClicked(x, y);
	}
	...
	do
	{
		...
		case TO_PLAY_MODE:
			...
			pOut->PrintPlayersInfo("P0(100, 0), P1(100,0), P2(100,0), P3(100,0)");
